...
var originalMenuCall = Scene_Map.prototype.callMenu;
Scene_Map.prototype.callMenu = function() {
    if (!this._menuCalled) {
        this._menuCalled = true; // Flag to prevent multiple calls
        _.loadAllImages(() => {
            if (SceneManager._scene instanceof Scene_Menu) {
                console.log("Menu was pushed prematurely, correcting stack...");
                SceneManager.pop(); // Pop the premature menu
            }
            originalMenuCall.call(this); // Push the menu properly after images are loaded
            this._menuCalled = false; // Reset the flag after menu is correctly handled
        });
    }
};
////////////////////////////new below////////////////////////////// vvvvvvvvvvvvvvvvvvv


///////////////////////////new above///////////////////////////// ^^^^^^^^^^^^^^^^^^^^^
var originalPush = SceneManager.push;
SceneManager.push = function(sceneClass) {
    console.log("Pushing scene:", sceneClass.name);
    originalPush.apply(this, arguments);
};

var originalPop = SceneManager.pop;
SceneManager.pop = function() {
    console.log("Popping scene:", (this._scene ? this._scene.constructor.name : "None"));
...
_.allImagesLoadedCallback = null;

_.loadAllImages = function(callback) {
    let imagesLoaded = 0;
    const totalImages = Object.keys(_.backgrounds).length;
    _.allImagesLoadedCallback = callback;
    
    for (const key in _.backgrounds) {
        if (_.backgrounds.hasOwnProperty(key)) {
            const bitmap = _.loadImage(_.backgrounds[key]);
            bitmap.addLoadListener(() => {
                imagesLoaded++;
                if (imagesLoaded === totalImages) {
                    console.log("All menu background images are loaded.");
                    if (_.allImagesLoadedCallback) {
                        _.allImagesLoadedCallback();
                    }
                }
            });
        }
...
_.SceneBoot_start = Scene_Boot.prototype.start;
Scene_Boot.prototype.start = function() {
    _.loadAllImages();
    _.SceneBoot_start.apply(this, arguments);     
};
